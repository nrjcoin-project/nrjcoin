...
#if defined(NDEBUG)
# error "Nrjcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock(NRJ_HASH_GENESIS_BLOCK);
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nrjcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nrjcoin Signed Message:\n";
...
{
    // Nrjcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
        nSubsidy = NRJ_BLOCKCOIN_PREMINING * COIN;
    else
        nSubsidy = NRJ_BLOCKCOIN * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Nrjcoin: 840k blocks in ~4 years

    if(nSubsidy<NRJ_BLOCKCOIN_MIN)
        nSubsidy=NRJ_BLOCKCOIN_MIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = NRJ_TARGETTIMESPAN; // Nrjcoin: 3.5 days to 3.0 days
static const int64 nTargetSpacing = NRJ_TARGETSPACING; // Nrjcoin: 2.5 minutes to 2.0 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nrjcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nrjcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Nrjcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256(NRJ_TESTNET_HASH_GENESIS_BLOCK);
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = NRJ_PSZTIMESTAMP;
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = NRJ_NTIME;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = NRJ_NNONCE;

        if (fTestNet)
        {
            block.nTime    = NRJ_NTIME;
            block.nNonce   = NRJ_TESTNET_NNONCE;
        }
        ...
        assert(block.hashMerkleRoot == uint256(NRJ_HASH_MERKLE_ROOT));


//        NrjSearchForGenesisblock(block);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Nrjcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NrjcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NrjcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NrjcoinMiner(CWallet *pwallet)
{
    printf("NrjcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nrjcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NrjcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NrjcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NrjcoinMiner, pwallet));
}
